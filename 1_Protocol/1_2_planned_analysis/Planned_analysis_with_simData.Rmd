...
This  script is used for testing the planned analyses in stage 1 registration. We tested the script on simulated data. As the simulated data are randomly sampled from a previous dataset, we should not interpret the result in any meaningful way.
...
## SPE index 1: RT1 
This SPE index only consider match condition.

Also, the SPE is indexed by the means RT differences between self and averaged others (friend & stranger).

We will focus on XXX when analyzing the real data
...
```
## SPE index 2: RT2
XXXX ( not distinguish match & nonmatch)
XXXX

(How will we interpret the results in the future)
...
```{r}
df.ddm <- read.csv("../2_3_SimData/test_retest_pilot_ddm_results.csv")
