...
list.SHR.MC.para <- mcshrgener(df.split = df.clean, iteration = 10, num_cores = 16)
# iteration = 100
# user      system    elapsed 
# 4.99      1.59      42.42 
# 880 MB
...
### Parallel Funcion
```{r SHR.MC of RT, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "rt")
```

```{r SHR.MC of ACC, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "acc")
```

```{r SHR.MC of Dprime, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "d")
```

```{r SHR.MC of Efficiency, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "eff")
```

```{r SHR.MC of DDM: v, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "ddmv")
```

```{r SHR.MC of DDM: z, message=FALSE, warning=FALSE}
mcshr(list = list.SHR.MC.para, nc = 16, indice = "ddmz")
```

### Plot SHR
```{r Code for Plot SHR, message= FALSE, warning = FALSE}
plot.SHR.MC$SPE_type<- factor(plot.SHR.MC$SPE_type, levels = c("RT", "ACC", "Dprime", "Eff", "ezDDM_v", "ezDDM_z"))
...
ggsave(filename="../1_2_Planned_Analysis/1_2_2_OUTPUT/Figure1_SPE_SHR.png", width = 9, height = 10)
```
...
ggplot(plot.ICC, aes(x = ICC_type, y = ICC, color = ICC_type), width = 12, height = 8) +
  ...
  theme(text = element_text(size = 20),
        ...
        strip.background = element_rect(color="black", fill="white", 
                                        size=1))  +
  guides(fill = guide_none(), color = guide_none())  #hide legend
ggsave(filename="../1_2_Planned_Analysis/1_2_2_OUTPUT/Figure2_SPE_ICC.png", width = 18, height = 10)
```
