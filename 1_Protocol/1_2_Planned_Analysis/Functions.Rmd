...
mcshr_rt <- function(list, nc) {
  ...
  r_values <- foreach(j = 1:length(list), .packages = c("dplyr", "tidyr")) %dopar% {
    ...
    SPE_half_1 <- list[[j]][[1]] %>%
      ...
      tidyr::pivot_wider(names_from = Identity,
                         values_from = mean_rt) %>%
      dplyr::mutate(rt_1_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
  ...
  cat("Estimated Indece: Reaction Time\n")
  cat("Monte-Carlo Split-Half Reliability:", round(r, 7), "\n")
  cat("95% CI:", paste(round(CI, 7), collapse=", "))
}
...
mcshr_acc <- function(list, nc) {
  ...
  r_values <- foreach(j = 1:length(list), .packages = c("dplyr", "tidyr")) %dopar% {
    ...
    SPE_half_1 <- list[[j]][[1]] %>%
      ...
      tidyr::pivot_wider(names_from = Identity,
                         values_from = acc) %>%
      dplyr::mutate(acc_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
...
mcshr_d <- function(list, nc) {
  ...
  r_values <- foreach(j = 1:length(list), .packages = c("dplyr", "tidyr")) %dopar% {
    ...
    tidyr::pivot_wider(names_from = Identity,
                       values_from = Dprime) %>%
    dplyr::mutate(dprime_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
...
mcshr_eff <- function(list, nc) {
  ...
  SPE_half_1 <- list[[j]][[1]] %>%
    ...
    tidyr::pivot_wider(names_from = Identity,
                       values_from = Eff) %>%
    dplyr::mutate(eff_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
  ...
  plot.SHR.MC[4,1] <<- "Efficiency"
  plot.SHR.MC[4,2] <<- r
  plot.SHR.MC[4,3] <<- CI[1]
  plot.SHR.MC[4,4] <<- CI[2]
  
  cat("Estimated Indece: Efficiency\n")
  cat("Monte-Carlo Split-Half Reliability:", round(r, 7), "\n")
  cat("95% CI:", paste(round(CI, 7), collapse=", "))
}
```
...
mcshr_ddmv <- function(list, nc) {
  ...
  SPE_half_1 <- list[[j]][[1]] %>%
    ...
    tidyr::pivot_wider(names_from = Identity,
                       values_from = v) %>%
    dplyr::mutate(v_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
    dplyr::select(v_SPE)
...
mcshr_ddmz <- function(list, nc) {
  ...
  r_values <- foreach(j = 1:length(list), .packages = c("dplyr", "tidyr", "hausekeep")) %dopar% {
    ...
    SPE_half_1 <- list[[j]][[1]] %>%
      ...
      tidyr::pivot_wider(names_from = Identity,
                         values_from = z) %>%
      dplyr::mutate(z_SPE = Self - rowMeans(select(., -(1:which(names(.)=="Self"))), na.rm = TRUE)) %>%
      dplyr::select(z_SPE)
