...
---
## Figure S1: SMT.pptx
## Figure S2: PRISMA.pptx

## Figure S3: DDM Packages Comparison
...
ggsave(
  plot = p_meta_bind,
  filename = "./FIGURE/supp/Fig_S4_meta_bind.png",
  width = 15,
  height = 6,
)
...
rm(df_close, df_stranger, df_meta_bind, p_meta_bind, p1, p2)
```

## Figure S5: Supp. SHR (per, fs, od)
```{r Aggregate Plot1}
aggr[[1]] <- plot[[1]] %>%
  dplyr::left_join(
    desc[[1]], by = "Paper_ID"
...
aggr[[2]] <- plot[[2]] %>%
  ...
  dplyr::mutate(
    ...
    LLCI = mapply(function(r, n) psych::r.con(
      rho = r, n = n, p = 0.95)[1], 
      r = r, 
      n = Sample_Size
    ),
    ULCI = mapply(function(r, n) psych::r.con(
      rho = r, n = n, p = 0.95)[2], 
      r = r, 
      n = Sample_Size
    ),
...
ggplot2::ggplot(., aes(x = reorder(Target_Indice, Order), y = wtmr)) +
  ...
  ggplot2::geom_hline(yintercept = 1, linetype = "dotted", color = "black", linewidth = 1, alpha = 0.5) + 
  ggplot2::geom_text(aes(y = -1, x = reorder(Target_Indice, Order), label = Target_Indice),
...
ggsave(plot = p0,
       filename = "./FIGURE/supp/Fig_S5_meta_per.png",
       width = 10,
       height = 8,
       )
ggsave(plot = p1,
       filename = "./FIGURE/supp/Fig_S5_meta_fs.png",
       width = 10,
       height = 8,
       )
ggsave(plot = p2,
       filename = "./FIGURE/supp/Fig_S5_meta_od.png",
       width = 10,
       height = 8,
       )
...
ggsave(plot = p1,
       filename = "./FIGURE/supp/Fig_S6a_ICC_P0.png",
       width = 10,
       height = 8,
       )

ggsave(plot = p2,
       filename = "./FIGURE/supp/Fig_S6b_ICC_Ps2.png",
       width = 10,
       height = 8,
       )
...
ggsave(plot = p,
       filename = "./FIGURE/supp/Fig_S6_ICC.png",
       width = 20,
       height = 8,
       )
...
```{r}
plot[[5]] <- read.csv("./OUTPUT/icc/icc_cov_ci.csv") %>%
  dplyr::mutate(
    Target = case_when(
      Target == "Friend" ~ "Close",
...
```{r}
p <- ggplot2::ggplot(data = plot[[5]], aes(x = stats::reorder(Target_Indice, Order), color = cov, y = ICC2)) +
  #ggplot2::geom_bar(stat = "Identity", position = "dodge") +
  ggplot2::geom_errorbar(
    aes(ymin = LLCI, ymax = ULCI),
    width = 0.1, linewidth = 1, position = position_dodge(width = 1)
  ) + # nolint
  ggplot2::geom_point(shape = 15, size = 3, position = position_dodge(width = 1)) +
  ggplot2::scale_y_continuous(
    limits = c(-0.4,1.1),
  ...
  ggplot2::geom_hline(yintercept = 1, linetype = "dotted", color = "black", linewidth = 1, alpha = 0.5) + # nolint
  ggplot2::geom_text(
    aes(y = -0.4, x = stats::reorder(Target_Indice, Order), label = Target_Indice),
    hjust = 0, size = 4, family='serif', color = "black"
  ) + # 
  ggplot2::scale_color_manual(values = c("grey30", "grey70")) +
  ggplot2::guides(fill = guide_legend(reverse = TRUE)) + # 改变图例顺序
  ggplot2::labs(
    color = "Covariate(BDI-II Scores)", 
    x = "", 
    y = "Intraclass Correlation Coefficient 2 (Absolute)"
  ) +
  ggplot2::coord_flip() +
  papaja::theme_apa() +
  ggplot2::theme(
...
ggsave(
  plot = p,
  filename = "./FIGURE/supp/Fig_S7_Cov_CI.png",
  width = 12,
  height = 8,
)
```

## Figure S8: Correlation SHR & Trials 
```{r}
plot[[6]] <- plot[[1]] %>%
...
ggsave(filename = "./FIGURE/supp/Fig_S8_r&Trial.png",
       width = 10,
       height = 8,
       )
#################################### SAVE ######################################
...
## Figure S9: Correlation Hedges'g & SHR
```{r}
temp_plot <- plot[[1]] %>%
  dplyr::filter(
...
ggplot2::ggsave(
  plot = p3,
  filename = "./FIGURE/supp/Fig_S9_yi&r.png",
  width = 10,
  height = 8,
)
...
## Figure S10: Correlation Hedges'g & Trials
```{r}
close <- read.csv("./OUTPUT/meta/meta_close_FEP.csv")
stranger <- read.csv("./OUTPUT/meta/meta_stranger_FEP.csv")
...
ggsave(
  filename = "./FIGURE/supp/Fig_S10_yi&trials.png",
  width = 10,
  height = 8,
)
...
rm(
  temp, wtm_r, wtm_t, steps, result_df,
...
ggsave(
  filename = "./FIGURE/supp/Fig_S11_Spear_Brown.png",
  width = 10,
  height = 8,
)
