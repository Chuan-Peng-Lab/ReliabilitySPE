---
title: "8_Meta_Plot"
author: "yuki"
date: "2023-09-07"
output: html_document
---
```{r}
out <- list(
  list(),
  list()
)

names(out)[1] <- "Self - Close"
names(out)[2] <- "Self - Stranger"
```

# Close
## RT
```{r}
out[[1]][[1]] <- res_meta[[1]][[1]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi*-1, 2),
    ci.lb = round(ci.lb*-1, 2),
    ci.ub = round(ci.ub*-1, 2),
    zi = round(zi*-1, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[1] <- "RT"
```

```{r}
ggplot2::ggplot(out[[1]][[1]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-3.5, 2.5), 
    breaks = seq(-0.5,2, by = 0.5),
    sec.axis = sec_axis(
      ~., 
      breaks = c(-2.9, -1.2, 2.3),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", linewidth = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -3.5, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.2, y = reorder(Unique_ID, Order), label = CI), size = 7, hjust = 0.5, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 2.3, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for RT (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_RT.png", width = 16, height = 9, limitsize = FALSE)
```

## ACC
```{r}
out[[1]][[2]] <- res_meta[[1]][[2]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[2] <- "ACC"
```

```{r}
ggplot2::ggplot(out[[1]][[2]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-3.6, 3.3), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.9, -1.1, 3),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -3.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 3, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for ACC (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_ACC.png", width = 16, height = 9, limitsize = FALSE)
```

## d
```{r}
out[[1]][[3]] <- res_meta[[1]][[3]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[3] <- "d"
```

```{r}
ggplot2::ggplot(out[[1]][[3]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.6, 1.6), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.17, -1, 1.5),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 1.5, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for d (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_d.png", width = 16, height = 9, limitsize = FALSE)
```

## eta
```{r}
out[[1]][[4]] <- res_meta[[1]][[4]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi*-1, 2),
    ci.lb = round(ci.lb*-1, 2),
    ci.ub = round(ci.ub*-1, 2),
    zi = round(zi*-1, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[4] <- "eta"
```

```{r}
ggplot2::ggplot(out[[1]][[4]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-4.5, 3.7), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-3.7, -1.3, 3.7),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -4.5, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.3, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 3.7, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for η (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_eta.png", width = 16, height = 9, limitsize = FALSE)
```

## v
```{r}
out[[1]][[5]] <- res_meta[[1]][[5]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[5] <- "v"
```

```{r}
ggplot2::ggplot(out[[1]][[5]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.6, 2.1), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.17, -1, 2),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 2, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for v (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_v.png", width = 16, height = 9, limitsize = FALSE)
```

## z
```{r}
out[[1]][[6]] <- res_meta[[1]][[6]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi*-1, 2),
    ci.lb = round(ci.lb*-1, 2),
    ci.ub = round(ci.ub*-1, 2),
    zi = round(zi*-1, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[1]])[6] <- "z"
```

```{r}
ggplot2::ggplot(out[[1]][[6]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.6, 1.3), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.2, -1, 1.2),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 1.2, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for z (Self - Close)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Close_z.png", width = 16, height = 9, limitsize = FALSE)
```

# Stranger
## RT
```{r}
out[[2]][[1]] <- res_meta[[2]][[1]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi*-1, 2),
    ci.lb = round(ci.lb*-1, 2),
    ci.ub = round(ci.ub*-1, 2),
    zi = round(zi*-1, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[1] <- "RT"
```

```{r}
ggplot2::ggplot(out[[2]][[1]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-3.5, 2.5), 
    breaks = seq(-0.5,2, by = 0.5),
    sec.axis = sec_axis(
      ~., 
      breaks = c(-2.9, -1.2, 2.3),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", linewidth = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -3.5, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.2, y = reorder(Unique_ID, Order), label = CI), size = 7, hjust = 0.5, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 2.3, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for RT (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_RT.png", width = 16, height = 9, limitsize = FALSE)
```

## ACC
```{r}
out[[2]][[2]] <- res_meta[[2]][[2]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[2] <- "ACC"
```

```{r}
ggplot2::ggplot(out[[2]][[2]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-3.6, 3.3), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.9, -1.1, 3.1),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -3.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 3.1, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for ACC (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_ACC.png", width = 16, height = 9, limitsize = FALSE)
```

## d
```{r}
out[[2]][[3]] <- res_meta[[2]][[3]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[3] <- "d"
```

```{r}
ggplot2::ggplot(out[[2]][[3]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.6, 1.6), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.17, -1, 1.5),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 1.5, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for d (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_d.png", width = 16, height = 9, limitsize = FALSE)
```

## eta
```{r}
out[[2]][[4]] <- res_meta[[2]][[4]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi*-1, 2),
    ci.lb = round(ci.lb*-1, 2),
    ci.ub = round(ci.ub*-1, 2),
    zi = round(zi*-1, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[4] <- "eta"
```

```{r}
ggplot2::ggplot(out[[2]][[4]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-4.7, 5.2), 
    breaks = seq(-0.5,5, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-3.7, -1.3, 5),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -4.7, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.3, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 5, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for η (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_eta.png", width = 16, height = 9, limitsize = FALSE)
```

## v
```{r}
out[[2]][[5]] <- res_meta[[2]][[5]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[5] <- "v"
```

```{r}
ggplot2::ggplot(out[[2]][[5]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.6, 1.6), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.17, -1, 1.5),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.6, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 1.5, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for v (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_v.png", width = 16, height = 9, limitsize = FALSE)
```

## z
```{r}
out[[2]][[6]] <- res_meta[[2]][[6]] %>%
  base::summary() %>% # 得到yi, vi, sei, zi, ci.lb, ci.ub这些变量
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::mutate(
    ############################################################################
    yi_r = round(yi, 2),
    ci.lb = round(ci.lb, 2),
    ci.ub = round(ci.ub, 2),
    zi = round(zi, 2),
    ############################################################################
    pval = ifelse(
      pval < 0.001, "<.001", 
      ifelse(
        pval < 0.05,
        sub("^0+", "", sprintf("%.3f", pval)),
        sub("^0+", "", sprintf("%.3f", pval))
      )
    ),
    LLCI = case_when(
      ci.lb < ci.ub ~ ci.lb,
      ci.lb > ci.ub ~ ci.ub,
    ),
    ULCI = case_when(
      ci.ub > ci.lb ~ ci.ub,
      ci.ub < ci.lb ~ ci.lb,
    ),
    CI = paste0(
      format(yi_r, nsmall = 2),
      "[", format(LLCI, nsmall = 2), ", ", format(ULCI, nsmall = 2), "]",
      sep = ""
    ),
    ############################################################################
    Unique_ID = paste0(Author," (",Year,")",", S",Study),
    ############################################################################
  ) %>%
  dplyr::arrange(yi_r) %>%
  dplyr::mutate(
    Order = row_number(),
  ) %>%
  dplyr::select(
    Paper_ID, Author, Year, Study,
    Unique_ID, Order,
    yi_r, vi, sei, zi, 
    LLCI, ULCI, CI, pval
  ) 

names(out[[2]])[6] <- "z"
```

```{r}
ggplot2::ggplot(out[[2]][[6]], aes(x = yi_r , y = reorder(Unique_ID, Order))) +
  ggplot2::geom_linerange(aes(xmin = LLCI, xmax = ULCI), linewidth = 1.5) +
  ggplot2::geom_point(shape=15, size = 5) +
  #ggplot2::geom_linerange(aes(xmin = fix_ci.lb, xmax = fix_ci.ub, y = 1.5), linewidth = 3, color = "grey") + # Fixed Effect CI
  #ggplot2::geom_point(aes(x = fix_yi, y = 1.5), shape=15, size = 7, color = "grey") + # Fixed Effect Point
  ggplot2::scale_x_continuous(
    limits = c(-2.8, 1.1), 
    breaks = seq(-0.5,3, by = 0.5),
    sec.axis = sec_axis(
      ~., breaks = c(-2.4, -1.2, 1),
      labels = c("Author(s), Year and Study", "SMD (95%-CI)", expression(italic(p) * "-value"))
    ),
  ) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 3, alpha = 0.5) +
  ggplot2::geom_text(aes(x = -2.8, y = reorder(Unique_ID, Order), label = reorder(Unique_ID, Order)), size = 7, hjust = 0, family='serif',) + # Author_Year_Study
  ggplot2::geom_text(aes(x = -1.2, y = reorder(Unique_ID, Order), label = CI),hjust = 0.5, size = 7, family='serif',) + # CI
  ggplot2::geom_text(aes(x = 1, y = reorder(Unique_ID, Order), label = pval), size = 7, hjust = 0.5, family='serif',) + # Right: P-value
  #ggplot2::annotate("text", x = -3.5, y = 1 ,label = "Q (161) = 1426.33, p <.0001, I2 = 90.90%", size = 16, family='serif', color = "black") +
  #ggplot2::annotate("text", x = -3, y = 2 ,label = "Random Effect = -0.88, SE = 0.05, p <.01, 95% CI [-.97, -.79]", size = 16, family='serif', color = "black") +
  ggplot2::labs(x = expression("Hedges' " * italic(g))) +
  ggplot2::ggtitle("Meta-Analysis for z (Self - Stranger)") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none",
                 axis.text.x.top = element_text(size = 20, family='serif'),
                 axis.text.x.bottom = element_text(size = 15, family='serif'),
                 axis.title.x = element_text(size = 20, hjust = 0.75),
                 plot.title = element_text(size = 20, family = 'serif'),
                 legend.title = element_blank(),
                 axis.line.y = element_blank(),
                 axis.ticks.x.top = element_line(color = "transparent"), #消除坐标轴上刻度凸起
                 axis.ticks.y= element_blank(),
                 axis.text.y= element_blank(),
                 axis.title.y= element_blank(),
                 ) 

ggsave(filename = "./FIGURE/Forest_Stranger_z.png", width = 16, height = 9, limitsize = FALSE)
```

# Correlation (yi & Trials)
```{r}
res_meta[[5]] <- dplyr::bind_rows(res_meta[[3]], res_meta[[4]]) %>%
  dplyr::mutate(
    Indice = case_when(
      Indice == "d" ~ "d'",
      Indice == "eta" ~ "η",
      Indice == "rwDDM v" ~ "v(RWiener)",
      Indice == "rwDDM z" ~ "z(RWiener)",
      Indice == "ezDDM v" ~ "v(hausekeep)",
      Indice == "ezDDM z" ~ "z(hausekeep)",
      TRUE ~ Indice
    ),
    yi = case_when(
      Indice == "RT" ~ yi * -1,
      Indice == "ACC" ~ yi,
      Indice == "d'" ~ yi,
      Indice == "η" ~ yi * -1,
      Indice == "v(hausekeep)" ~ yi,
      Indice == "z(hausekeep)" ~ yi * -1,
      Indice == "v(RWiener)" ~ yi,
      Indice == "z(RWiener)" ~ yi * -1, 
    ),
    Target = factor(
      Target,levels = c("Close", "Stranger")
    ),
    Indice = factor(
      Indice, levels = c("RT", "ACC", "d'", "η", "v(RWiener)", "z(RWiener)", "v(hausekeep)", "z(hausekeep)")
    )
  )
names(res_meta)[5] <- "Correlation (yi & Trials)"
```

```{r}
pic1 <- res_meta[[5]] %>%
  dplyr::filter((Indice %in% c("RT", "ACC", "d'", "η"))) %>%
  ggplot2::ggplot(data = ., aes(x = Trials, y = yi, color = Indice, fill = Indice)) +
  ggplot2::geom_point() +  # 添加散点
  ggplot2::geom_smooth(method = "lm", se = TRUE, fullrange = TRUE) +  # 添加拟合曲线
  ggpubr::stat_cor(label.x = 3, label.y = 1.1, r.accuracy = .01, p.accuracy = .01) +
  ggplot2::scale_x_continuous(
    limits = c(0, 140),
    breaks = c(0, 30, 60, 100, 130),
  ) +
  ggplot2::scale_y_continuous(
    limits = c(-0.5, 2),
    breaks = c(0, 0.5, 1, 1.5, 2)
  ) +
  ggplot2::facet_grid(cols = vars(Target), rows = vars(Indice)) +
  ggplot2::scale_color_manual(values = c("#F8766D", "#CD9600", "#7AAC00", "#03BE66")) +
  ggplot2::scale_fill_manual(values = c("#F8766D", "#CD9600", "#7AAC00", "#03BE66")) +
  ggplot2::labs(y = expression("Hedges' " * italic(g)), x = "Trial Numbers") +  
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none")

pic2 <- res_meta[[5]] %>%
  dplyr::filter((Indice %in% c("v(RWiener)", "z(RWiener)", "v(hausekeep)", "z(hausekeep)"))) %>%
  ggplot2::ggplot(data = ., aes(x = Trials, y = yi, color = Indice, fill = Indice)) +
  ggplot2::geom_point() +  # 添加散点
  ggplot2::geom_smooth(method = "lm", se = TRUE, fullrange = TRUE) +  # 添加拟合曲线
  ggpubr::stat_cor(label.x = 3, label.y = 1.1, r.accuracy = .01, p.accuracy = .01) +
  ggplot2::scale_x_continuous(
    limits = c(0, 140),
    breaks = c(0, 30, 60, 100, 130),
  ) +
  ggplot2::scale_y_continuous(
    limits = c(-0.5, 2),
    breaks = c(0, 0.5, 1, 1.5, 2)
  ) +
  ggplot2::facet_grid(cols = vars(Target), rows = vars(Indice)) +
  ggplot2::scale_color_manual(values = c("#00BFC4", "#00A8FF", "#C678FF", "#FF60CC")) +
  ggplot2::scale_fill_manual(values = c("#00BFC4", "#00A8FF", "#C678FF", "#FF60CC")) +
  ggplot2::labs(y = "", x = "Trial Numbers") +
  papaja::theme_apa() +
  ggplot2::theme(legend.position = "none")
  
pic <- pic1 + pic2 + plot_layout(guides = 'collect')

ggsave(
  filename = "./FIGURE/Fig7_Correlation.png",
  width = 10,
  height = 8,
)

rm(pic1, pic2, pic)
```

```{r}
spearman_cor <- res_meta[[5]] %>%
  dplyr::group_by(Target, Indice) %>%
  dplyr::summarise(cor = cor(Trials, yi, method = "spearman"))
```

