...
```{r}
res_meta[[3]] <- read.csv("./OUTPUT/meta/meta_close_FEP.csv")
res_meta[[4]] <- read.csv("./OUTPUT/meta/meta_stranger_FEP.csv")
...
pic1 <- desc[[8]] %>%
  ...
  dplyr::mutate(
    Trial_Expect = case_when(
      Trial_Expect >= 300 ~ 300,
      TRUE ~ Trial_Expect
    ),
    Over = case_when(
      Trial_Expect == 300 ~ T,
  ...
  ggplot2::ggplot(
    data = ., 
    mapping = aes(
      x = r_Expect, y = Trial_Expect, 
      color = r_round, group = r_round
    )
  ) +
  ggplot2::geom_point(
    aes(shape = ifelse(Over, "T", 'F')), 
    size = 1, 
    position = position_dodge(width = 0.2)
  ) +
  ggplot2::scale_shape_manual(values = c("T" = 21, "F" = 16), guide = FALSE) +
  ggplot2::geom_smooth(
    data = . %>% filter(abs(r_round - wtmr_round) <= 0.1),
    mapping = aes(x = r_Expect, y = Trial_Expect, group = NULL),
  ...
  ggplot2::facet_grid(cols = vars(Target), rows = vars(Indice)) +
  ggplot2::scale_color_gradient(low = "#b2182b", high = "#2166ac", breaks = seq(0, 1, by = 0.1)) +
  ggplot2::labs(
    y = "Expected Trial Numbers", 
    x = "Expected Spli-Half Reliability",
    color = "Original r"
  ) +
  papaja::theme_apa()
