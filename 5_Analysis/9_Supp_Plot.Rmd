...
for (i in 1:99) {
  ...
  # 生成列名
  trial_col <- paste0("Trial_", i)
  llci_col <- paste0("LLCI_", i)
  ulci_col <- paste0("ULCI_", i)
  
  # 计算并添加列
  plot[[9]][[trial_col]] <- abs((ratio * wtm_r - ratio) / (ratio * wtm_r - wtm_r) * wtm_t)
  plot[[9]][[llci_col]] <- abs((ratio * ULCI - ratio) / (ratio * ULCI - ULCI)) * wtm_t
  plot[[9]][[ulci_col]] <- abs((ratio * LLCI - ratio) / (ratio * LLCI - LLCI) * wtm_t)
}

rm(temp, result_df, element_name, i, wtm_r, wtm_t, LLCI, ULCI)
...
pic1 <- plot[[9]] %>%
  ...
  ggplot2::geom_ribbon(
    aes(ymin = ULCI, ymax = LLCI, fill = Indice),
