...
df[[21]] <- read.mat(list = list_mat_e1) %>%
  dplyr::mutate(
    Matching = case_when(Shape == Label ~ "Matching",
                         Shape != Label ~ "Nonmatching"),
    Identity = case_when(Shape == 1 ~ "Self",
                         Shape == 2 ~ "Friend",
                         Shape == 3 ~ "Stranger"),
    RT_ms = RT,
    RT_sec = RT / 1000,
...
# 获取文件夹内所有满足条件的文件名
file_list <- list.files(path = "../4_Data_Extraction/4_2_Open_Data/Paper_s4/Source/", pattern = "^PractExperiment_2.*\\.mat$", full.names = TRUE)
