...
shl[[4]] <- yukiSH::sh_tool(df.split = df[[i]], method = "mc",
                            sub = "Subject", var1 = "Matching", var2 = "Identity", var3 = "Session",
                            iteration = 5000, nc = nc)
```

```{r Run!}
# 计算8个参数的分半信度 
print("start rt")
df_rt <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "rt", mc = TRUE, nc = nc)
print("start acc")
df_acc <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "acc", mc = TRUE, nc = nc)
print("start dp")
df_dp <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "dp", mc = TRUE, nc = nc)
print("start eff")
df_eff <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "eff", mc = TRUE, nc = nc)
print("start ezddm")
df_ezddm <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "ezddm", mc = TRUE, nc = nc)
print("start rwddm")
df_rwddm <- yukiBP::shr(list = shl[[4]], Target = t[[i]], Paper_ID = p[[i]], Indice = "rwddm", mc = TRUE, nc = nc)
```
