...
ggsave(filename = "./FIGURE/Fig2_meta_mcshr_unite_ez.png",
       width = 10,
       height = 8,
       )
```
...
```{r, message=FALSE}
for (i in 1:18) {
  raw_meta[[i]][[1]] <-df.rt(df = df[[i]], i = i)
  raw_meta[[i]][[2]] <-df.acc(df = df[[i]], i = i)
  raw_meta[[i]][[3]] <-df.dp(df = df[[i]], i = i)
  raw_meta[[i]][[4]] <-df.eta(df = df[[i]], i = i)
  #raw_meta[[i]][[5]] <-df.ezddm(df = df[[i]], i = i)
  raw_meta[[i]][[5]] <-df.rwddm(df = df[[i]], i = i)
  
  raw_meta[[i]] <- base::Reduce(function(x, y) left_join(x, y, by = c("Identity", "Paper_ID")), raw_meta[[i]])
}
...
df_rt <- df[[1]] %>%
  ...
  dplyr::summarise(RT = mean(mean_rt)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(Paper_ID = p[[1]])
