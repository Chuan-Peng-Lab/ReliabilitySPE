...
```{r}
ggplot2::ggplot(meta[[1]], aes(x = Indice, y = yi, color = interaction(Indice, Target))) +
  ggplot2::geom_linerange(aes(ymin = ci.lb, ymax = ci.ub, color = interaction(Indice, Target)), size = 1) +
  ggplot2::geom_point(aes(color = interaction(Indice, Target)), shape=15, size = 2) +
...
plot[[3]] <- list.files(file.path("./OUTPUT/icc/"),
  ...
  dplyr::filter(Paper_ID == "P0E1") %>%
  dplyr::mutate(type = factor(type, levels = c("ICC2k", "ICC2")),
                Target = factor(Target,levels = c("Close", "Stranger", "Celebrity", "NonPerson")),
...
ggplot2::ggplot(plot[[3]], aes(x = reorder(Target_Type, Order), y = ICC, group = Paper_ID, color = type)) + # nolint
