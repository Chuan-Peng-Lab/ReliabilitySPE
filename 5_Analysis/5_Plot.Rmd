...
```{r}
ggplot2::ggplot(meta[[2]], aes(x = Indice, y = yi, color = interaction(Indice, Target))) +
  ggplot2::geom_linerange(aes(ymin = ci.lb, ymax = ci.ub, color = interaction(Indice, Target)), size = 1) +
  ggplot2::geom_point(aes(color = interaction(Indice, Target)), shape = 15, size = 3) +
  ggplot2::coord_flip() + #flips the x and y axes of a plot
  ggplot2::scale_x_discrete() +
  ggplot2::scale_y_continuous(limits = c(-0.1, 1.1),
                              breaks = c(0, 0.5, 0.7, 1),
...
ggsave(filename = "./FIGURE/Fig2_meta_mcshr.png",
       width = 10,
       height = 7,
       )
```
