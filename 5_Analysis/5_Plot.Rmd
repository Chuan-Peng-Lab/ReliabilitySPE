...
meta[[1]] <- plot[[1]] %>%
  ...
  dplyr::mutate(.,
                Indice = case_when(Indice == "d Prime" ~ "d'",
                                   #Indice == "rwDDM v" ~ "v",
                                   #Indice == "rwDDM z" ~ "z",
...
ggsave(filename = "./FIGURE/Fig1_meta_nmshr.png",
       width = 30,
       height = 15,
       )
...
meta[[2]] <- plot[[2]] %>%
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::filter(!(Paper_ID %in% c("P51E1", "P95E1", "P95E2", "PS1E1"))) %>%
...
ggsave(filename = "./FIGURE/Fig2_meta_mcshr_unite_ez_filter.png",
       width = 10,
       height = 8,
       )
```
...
plot[[3]] <- list.files(file.path("./OUTPUT/icc/"),
  ...
  dplyr::filter((Paper_ID %in% c("P0E1", "Ps2E1"))) %>%
  dplyr::mutate(type = factor(type, levels = c("ICC2", "ICC2k")),
                ...
                #ICC_type = factor(ICC_type,
                                  ...
                                  #),
                ICC_type = case_when(ICC_type == "d Prime" ~ "d'",
                                     #ICC_type == "rwDDM v" ~ "v",
                                     #ICC_type == "rwDDM z" ~ "z",
...
```{r}
plot[[3]] %>%
  dplyr::mutate(ICC_type = factor(ICC_type, levels = c("z(hausekeep)", "v(hausekeep)","z(RWiener)", "v(RWiener)", "Efficiency", "d'", "ACC", "RT"))) %>%
  ggplot2::ggplot(., aes(x = ICC_type, y = ICC, group = Paper_ID, color = interaction(ICC_type, Target))) + # nolint
...
ggsave(filename = "./FIGURE/Fig3_icc_supp.png",
       width = 15,
       height = 7,
       )
```
...
plot[[3]] %>%
  ...
  ggplot2::geom_errorbar(aes(color = type, ymin = LLCI, ymax = ULCI), width = 0.1, size = 1, position = position_dodge(width = 1)) + # nolint
  ggplot2::geom_point(shape = 15, size = 3, position = position_dodge(width = 1)) +
  ggplot2::coord_flip() + #flips the x and y axes of a plot
  ggplot2::scale_x_discrete() +
  ggplot2::scale_y_continuous(limits = c(-0.3, 1.3),
                              breaks = c(0, 0.5, 0.7, 1),
                              sec.axis = sec_axis(~., breaks = c(0, 0.5, 0.7, 1),
                                                  labels = c("Unacceptable", "Poor", "Acceptable", "Excellent")),
                              ) +
  ...
  ggplot2::geom_hline(yintercept = 0.7, linetype = "dotted", color = "black", size = 1, alpha = 0.5) + # nolint
  ggplot2::geom_hline(yintercept = 1, linetype = "dotted", color = "black", size = 1, alpha = 0.5) + # nolint
  ggplot2::scale_fill_manual(values = c("grey60", "black")) +
  ggplot2::scale_color_manual(values = c("grey60", "black")) +
...
ggsave(filename = "./FIGURE/Fig3_icc_unite.png",
       width = 10,
       height = 7,
       )
```
...
plot[[5]] <- plot[[2]] %>%
  dplyr::left_join(desc[[1]], by = "Paper_ID") %>%
  dplyr::filter(Target == "Close" | Target == "Stranger") %>%
  dplyr::filter(!(Paper_ID %in% c("Ps1E1", "P95E1", "P95E2'", "P51E1"))) %>%
  dplyr::mutate(r_8 = case_when(Target == "Close" & Indice == "RT" ~ 115,
                                Target == "Stranger" & Indice == "RT" ~ 75,
                                Target == "Close" & Indice == "ACC" ~ 125,
                                Target == "Close" & Indice == "Efficiency" ~ 105,
                                Target == "Stranger" & Indice == "Efficiency" ~ 80,
                                Target == "Close" & Indice == "ezDDM v" ~ 120,
