...
icc_output <- function(list) {
  ...
  final$ICC_type <- rep(c("RT","ACC","Dprime","Efficiency"), 
                        each = nrow(final)/4)
  ...
  final <- final %>%
    dplyr::mutate(.,ICC_type = factor(ICC_type, 
                                      levels = c("RT","ACC","Dprime","Efficiency")) ) %>%
    dplyr::arrange(.,type) %>%
...
# ICC: Self - Friend
```{r Self - Friend}
df.rt <- batch::icc(df = df[[1]], indice = "rt", Target = "Friend")

df.acc <- batch::icc(df = df[[1]],indice = "acc", Target = "Friend")

df.dp <- batch::icc(df = df[[1]],indice = "dp", Target = "Friend")

df.eff <- batch::icc(df = df[[1]],indice = "eff", Target = "Friend")
```

```{r DF for Plot ICC}
iccl[[1]] <- list(df.rt, df.acc, df.dp, df.eff) 
...
```{r Save icc_friend}
write.csv(res[[3]][[1]], "./OUTPUT/icc_friend.csv", row.names = FALSE)
```
--------------------------------------------------------------------------------
...
```{r DF for Plot ICC}
iccl[[2]] <- list(df.rt, df.acc, df.dp, df.eff) 
...
```{r Save icc_stranger}
write.csv(res[[3]][[1]], "./OUTPUT/icc_stranger.csv", row.names = FALSE)
```